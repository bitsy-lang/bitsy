...
impl Expr {
    ...
    fn rebase_rec(&self, current_path: Path, shadowed: &BTreeSet<Path>) -> Arc<Expr> {
        Arc::new(match self {
            ...
            Expr::VecDynIdx(loc, typ, e, i) => Expr::VecDynIdx(loc.clone(), typ.clone(), e.rebase_rec(current_path.clone(), shadowed), i.rebase_rec(current_path.clone(), shadowed)),
            Expr::VecUpdateDynIdx(loc, typ, e, i, ei) => Expr::VecUpdateDynIdx(
                loc.clone(),
                typ.clone(),
                e.rebase_rec(current_path.clone(), shadowed),
                i.rebase_rec(current_path.clone(), shadowed),
                ei.rebase_rec(current_path.clone(), shadowed),
            ),
    ...
    fn references_to_nets_rec(&self, net_id_by_path: &BTreeMap<Path, NetId>, shadowed: &BTreeSet<Path>) -> Arc<Expr> {
        Arc::new(match self {
            ...
            Expr::VecDynIdx(loc, typ, e, i) => Expr::VecDynIdx(loc.clone(), typ.clone(), e.references_to_nets_rec(net_id_by_path, shadowed), i.references_to_nets_rec(net_id_by_path, shadowed)),
            Expr::VecUpdateDynIdx(loc, typ, e, i, ei) => Expr::VecUpdateDynIdx(
                loc.clone(),
                typ.clone(),
                e.references_to_nets_rec(net_id_by_path, shadowed),
                i.references_to_nets_rec(net_id_by_path, shadowed),
                ei.references_to_nets_rec(net_id_by_path, shadowed),
            ),
    ...
    fn depends_on_net(&self, net_id: NetId) -> bool {
        match self {
            ...
            Expr::VecDynIdx(_loc, _typ, e, i) => e.depends_on_net(net_id) || i.depends_on_net(net_id),
            Expr::VecUpdateDynIdx(_loc, _typ, e, i, ei) => e.depends_on_net(net_id) || i.depends_on_net(net_id) || ei.depends_on_net(net_id),
